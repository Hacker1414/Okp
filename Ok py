...
#-------------[ CRACK-FROM-FILE ]------------------#
def crack_file():
	try:vin = os.listdir('/sdcard/TAJUL-ISLAM')
	except FileNotFoundError:
	...
	if len(vin)==0:
		...
		lol = {}
		for isi in vin:
			try:hem = open('/sdcard/TAJUL-ISLAM/'+isi,'r').readlines()
			except:continue
			cih+=1
			if cih<100:
				nom = ''+str(cih)
		...
		try:geh = lol[geeh]
			...
			time.sleep(3)
			back()
		try:lin = open('/sdcard/TAJUL-ISLAM/'+geh,'r').read().splitlines()
		except:
...
if __name__=='__main__':
	...
	try:os.mkdir('/sdcard/TAJUL-ISLAM')
	except:pass
